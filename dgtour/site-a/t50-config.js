...
const T50_BIZCFG_KEY='dtidA_t50BizCfg';
...
const T50_PERSONA_KEY='dtidA_admPersona';
function admPersona(){
...
const T50_MYQ_PREFIX='dtidA_t50MyQueue_';
function t50AppKeys(){
  const ks=[];
  for(let i=0;i<localStorage.length;i++){
    const k=localStorage.key(i);
    if(k&&(k.indexOf(T50_AP_PREFIX)===0||k==='dtidA_t50Applies'))ks.push(k);
  }
  return ks;
}
...
const T50_SCREEN_KEY='dtidA_t50ScreenCfg';
...
const T50_LOCK_PREFIX='dtidA_t50Lock_';
...
/* ── 시안 공통 정책 저장소 ────────────────────────────────────
   ...
   시안 접두사(dtidA_)를 유지한다. */
const T50_GRANT_KEY='dtid_t50GrantCfg';       /* 지원금액·환급률·사용기한 */
const T50_UNIFIED_KEY='dtid_t50UnifiedCfg';   /* 54개 항목 중 '통합' 항목 */
const T50_OPT_KEY='dtid_t50OptCfg';           /* 54개 항목 중 '옵션' 항목 (지역별) */
/* 공통 키로 옮기기 전에 이 시안에 저장해 둔 값이 있으면 한 번만 가져온다.
   먼저 열린 시안의 값이 채택되며, 이후에는 공통 키만 쓴다. */
function t50AdoptLegacy(shared,legacy){
  try{
...
t50AdoptLegacy(T50_GRANT_KEY,'dtidA_t50GrantCfg');
t50AdoptLegacy(T50_UNIFIED_KEY,'dtidA_t50UnifiedCfg');
...
const T50_CFG_SCHEMA=[
 {group:'대상 · 신청 조건',items:[
  ...
  {k:'closeRule',   lb:'마감 기준',        scope:'option', type:'select',opts:['예산 소진','선착순'],def:'예산 소진',
   by:'t50CloseState',note:'예산 소진 9곳 최다 · 확정 방침 「예산 기준」 — 예산 총액 도달 시 접수 마감'},
  ...
  {k:'dtidRequired',lb:'디지털관광주민증',  scope:'unified',type:'toggle',on:'필수',off:'선택',def:true,
   by:'t50DtidCheck',note:'확정 방침 — 주민증 서비스 가입자만 신청 가능(비로그인 접수 차단)'}
 ]},
 {group:'정산 · 지급',items:[
...
function t50OptCfg(){
  try{const c=JSON.parse(localStorage.getItem(T50_OPT_KEY)||'null');return (c&&typeof c==='object')?c:{};}
  catch(e){return {};}
}
function t50SaveOptCfg(c){localStorage.setItem(T50_OPT_KEY,JSON.stringify(c||{}));}
function t50AdoptLegacyOpt(){
  try{
    if(localStorage.getItem(T50_OPT_KEY)!==null)return;
    const all=t50Cfg(),out={};
    Object.keys(all).forEach(function(n){ if(all[n]&&all[n].opt)out[n]=all[n].opt; });
    if(Object.keys(out).length)t50SaveOptCfg(out);
  }catch(e){}
}
t50AdoptLegacyOpt();
...
function t50CloseState(region){
  const cfg=t50RegionCfg(region);
  const rule=t50CfgVal(region,'closeRule');
  if(rule==='선착순'){
    const used=t50UsedCount(region);
    return {rule:rule,closed:used>=cfg.capacity,used:used,cap:cfg.capacity,
      msg:'선착순 — 정원 '+cfg.capacity+'명 중 '+used+'명 접수'};
  }
  const bud=Number(cfg.budget)||0, spent=t50BudgetUsed(region);
...
function t50DtidCheck(region,uid){
  if(!t50CfgVal(region,'dtidRequired'))return null;
